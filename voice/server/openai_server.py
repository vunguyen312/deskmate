...
Usage:
    ...
    python server/openai_server.py --voices voices.json
...
def main():
    ...
    # Capture CUDA graphs before the port binds so the first speech request
    # never pays the one-time capture or queues behind it. On failure the
    # lazy capture on the first request still applies (_warmed_up stays False).
    try:
        tts_model.warmup(prefill_len=100)
        logger.info("CUDA graphs captured and ready")
    except Exception:
        logger.warning(
            "CUDA graph capture failed; will capture lazily on first request",
            exc_info=True,
        )
